...
impl<T: ?Sized> StandardBloomFilter<T> {
    ...
    /// Return the indexes corresponding to the items
    pub fn item_to_indexes(&mut self, item: &T) -> Vec<usize>
    where
        T: Hash,
    {
        let (h1, h2) = self.hash_kernel(item);
        let n_hash = self.optimal_k as usize;
		let mut v: Vec<usize> = Vec::with_capacity(n_hash);
        for k_i in 0..n_hash {
            let index = self.get_index(h1, h2, k_i as u64);
			v.push(index);
        }
